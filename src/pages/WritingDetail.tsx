	...
	const breadcrumbJsonLd = {
		...
		itemListElement: [
			...
			},
			{
				"@type": "ListItem",
				position: 3,
				name: articleData.metadata.title,
				item: canonicalUrl,
			},
		],
	};
			...
			<div className='bg-olive-50 dark:bg-olive-800 relative size-full min-h-screen'>
				<div className='flex flex-col gap-6 items-start mx-auto px-4 py-10 w-full max-w-xl'>
					...
					<div className={`${animateClass("animate-delay-1")} w-full`}>
						<ArticleHeader title={articleData.metadata.title} />
					</div>

					{/* Article Content */}
					<div
						className={`flex flex-col items-start relative shrink-0 w-full ${animateClass("animate-delay-2")}`}>
						<ReactMarkdown
