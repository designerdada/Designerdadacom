...
export function PhotoUploadForm({ onUploadComplete }: PhotoUploadFormProps) {
	...
	return (
		<form onSubmit={handleSubmit} className='flex flex-col gap-6'>
			{/* File upload area */}
			<div>
				<label className='flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-olive-500 dark:border-olive-400/30 px-4 py-8 transition-colors hover:border-olive-950 dark:hover:border-olive-100'>
					<Upload className='mb-2 size-8 text-olive-500 dark:text-olive-400' />
					<span className='text-xs text-olive-500 dark:text-olive-400'>
						{hasPhotos ? "Click to add more photos" : "Click to upload photos"}
					</span>
					<span className='mt-1 text-xs text-olive-500 dark:text-olive-400/60'>
							...
							<div
								...
								className={`relative rounded-lg border ${
									photo.uploaded
										...
										: "border-olive-500 dark:border-olive-400/30"
								}`}>
								...
								<div className='p-2'>
									<input
										...
										className='w-full rounded border border-olive-500 dark:border-olive-400/30 bg-transparent px-2 py-1 text-xs text-olive-800 dark:text-olive-100 placeholder:text-olive-500 dark:text-olive-400/50 focus:border-olive-950 dark:focus:border-olive-100 focus:outline-none disabled:opacity-50'
									/>
									<div className='mt-1 text-[10px] text-olive-500 dark:text-olive-400/70'>
										{camera} · {film}
									</div>
								</div>
							</div>
			...
			<div className='grid grid-cols-2 gap-4'>
				<div>
					<label className='mb-1 block text-xs text-olive-500 dark:text-olive-400'>
						Camera (all photos)
					</label>
					<select
						value={camera}
						...
						disabled={isUploading}
						className='w-full rounded-lg border border-olive-500 dark:border-olive-400/30 bg-transparent px-3 py-2 text-olive-800 dark:text-olive-100 focus:border-olive-950 dark:focus:border-olive-100 focus:outline-none disabled:opacity-50'>
						{CAMERAS.map((cam) => (
			...
			{uploadProgress && (
				<div className='flex flex-col gap-2'>
					<div className='h-2 overflow-hidden rounded-full bg-olive-500 dark:bg-olive-400/20'>
						<div
							className='h-full bg-olive-950 dark:bg-olive-100 transition-all'
