...
export const H1 = ({ children }: ComponentProps) => (
  <h1 className="font-medium leading-normal not-italic text-[var(--foreground)] text-2xl text-justify w-full pb-4 pt-2">
    {children}
  </h1>
);

export const H2 = ({ children }: ComponentProps) => (
  <h2 className="font-semibold leading-normal not-italic text-[var(--foreground)] text-2xl text-justify w-full pb-3 pt-6">
    {children}
  </h2>
);

export const H3 = ({ children }: ComponentProps) => (
  <h3 className="font-semibold leading-normal not-italic text-[var(--foreground)] text-2.5xl text-justify w-full pb-2 pt-4">
    {children}
  </h3>
);
...
export const P = ({ children }: ParagraphProps) => {
  ...
  if (isFirstPara && children) {
    ...
    return (
      <p className="font-normal leading-normal not-italic text-[var(--foreground)] text-lg text-justify w-full pb-4">
        {processedChildren}
      </p>
    );
  }
...
export const Blockquote = ({ children }: ComponentProps) => (
  <div className="box-border content-stretch flex flex-col gap-1 items-start justify-center pl-4 pr-0 py-2 relative shrink-0 w-full border-l-4 border-[var(--foreground)] my-4">
    <blockquote className="italic leading-normal relative shrink-0 text-[var(--foreground)] text-lg w-full [&>div]:pb-0 [&>div]:pt-0 [&_p]:italic [&_p:only-child]:pb-0">
      {children}
    </blockquote>
  </div>
);
...
export const Img = ({ src, alt, title }: ImageProps) => (
  <div className="box-border content-stretch flex flex-col gap-2 items-start justify-center px-0 py-6 relative shrink-0 w-full">
    ...
    {title && (
      <p className="leading-normal not-italic relative shrink-0 text-sm text-center text-[var(--muted)] w-full">
        {title}
      </p>
    )}
  </div>
