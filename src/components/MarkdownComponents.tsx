...
export const Pre = ({ children }: ComponentProps) => (
	<pre className='bg-olive-100 dark:bg-olive-900 p-4 rounded overflow-x-auto text-sm mb-4'>
		{children}
	</pre>
);
...
export const Img = ({ src, alt, title }: ImageProps) => (
	<div className='flex flex-col gap-2 items-start justify-center px-0 py-6 relative shrink-0 w-full'>
		...
		{title && (
			<p className='relative shrink-0 text-xs text-center text-olive-500 dark:text-olive-400 w-full'>
				{title}
			</p>
		)}
	</div>
	...
	return (
		<div className='flex items-start justify-center px-0 py-6 relative shrink-0 w-full'>
			<div
				...
				style={{ paddingBottom: isYouTube ? "56.25%" : "0", height: isYouTube ? "0" : "auto" }}>
				<iframe
					src={src}
					...
					className={
						isYouTube ? "absolute top-0 left-0 w-full h-full rounded-lg" : "w-full rounded-lg"
					}
