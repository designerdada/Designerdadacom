...
export const H1 = ({ children }: ComponentProps) => (
  <h1 className="font-medium leading-[1.5] not-italic text-[var(--foreground)] text-[24px] text-justify w-full pb-4 pt-2">
    {children}
  </h1>
);

export const H2 = ({ children }: ComponentProps) => (
  <h2 className="font-semibold leading-[1.5] not-italic text-[var(--foreground)] text-[24px] text-justify w-full pb-3 pt-6">
    {children}
  </h2>
);

export const H3 = ({ children }: ComponentProps) => (
  <h3 className="font-semibold leading-[1.5] not-italic text-[var(--foreground)] text-[22px] text-justify w-full pb-2 pt-4">
    {children}
  </h3>
);
  ...
  if (isFirstPara && children) {
    ...
    const processChildren = (child: any): any => {
      if (typeof child === 'string' && child.trim().length > 0) {
        ...
        return (
          <>
            <span className="float-left text-[50px] leading-none font-medium mr-1 -mt-1 text-[var(--foreground)]">
              {firstLetter}
            </span>
            {rest}
          </>
    ...
    return (
      <p className="font-normal leading-[1.5] not-italic text-[var(--foreground)] text-[18px] text-justify w-full pb-4">
        {processedChildren}
      </p>
    );
  }
...
export const UL = ({ children }: ComponentProps) => (
  <ul className="font-normal leading-[0] not-italic text-[var(--foreground)] text-[18px] text-justify w-full list-disc pl-6 pb-4">
    {children}
  </ul>
);

export const OL = ({ children }: ComponentProps) => (
  <ol className="font-normal leading-[0] list-decimal not-italic text-[var(--foreground)] text-[18px] text-justify w-full pl-6 pb-4">
    {children}
  </ol>
);
...
export const Blockquote = ({ children }: ComponentProps) => (
  <div className="box-border content-stretch flex flex-col gap-1 items-start justify-center pl-4 pr-0 py-2 relative shrink-0 w-full border-l-4 border-[var(--foreground)] my-4">
    <blockquote className="italic leading-[1.5] relative shrink-0 text-[var(--foreground)] text-[18px] w-full [&>div]:pb-0 [&>div]:pt-0 [&_p]:italic [&_p:only-child]:pb-0">
      {children}
    </blockquote>
  </div>
);
...
export const Code = ({ children }: ComponentProps) => (
  <code className="bg-[var(--accent)] px-1.5 py-0.5 rounded text-[16px]">
    {children}
  </code>
);
...
export const Pre = ({ children }: ComponentProps) => (
  <pre className="bg-[var(--accent)] p-4 rounded overflow-x-auto text-[16px] mb-4">
    {children}
  </pre>
);
...
export const Img = ({ src, alt, title }: ImageProps) => (
  <div className="box-border content-stretch flex flex-col gap-2 items-start justify-center px-0 py-6 relative shrink-0 w-full">
    ...
    {title && (
      <p className="leading-[1.5] not-italic relative shrink-0 text-[14px] text-center text-[var(--muted)] w-full">
        {title}
      </p>
    )}
  </div>
